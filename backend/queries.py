...
def book_appointment_with_auto_message(patient_id, date, time, status=None, reason=None):
    try:
...
def create_patient_appt_summary_view():
    ...
    SELECT a.appointment_id, a.patient_id, p.name AS patient_name, d.name AS doctor_name,
           a.date, a.time, a.status, a.reason
    FROM appointment a
...
def main():
    ...
    book_appointment_with_auto_message(
        patient_id=1,
        date="2025-05-15",
        time="14:30:00"
    )
